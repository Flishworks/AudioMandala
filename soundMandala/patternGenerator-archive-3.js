...
var numReflect=4; //changes the amount of lines to draw. 1=1, 2=2, 3=4, 4=8, 0=orchidstyle
var numRotate=8;
var patternX=50;
var patternY=50;
...
var lineFreq=4;
var outlineWidth=5;
var reflection=false, rotation=true; //flags for drawing mode
...
function genPath(){ //modulation schemes
  if (mouseIsPressed){
    ...
    switch (pattern){
      ...
      case 3:
        //atomic
       patternX=width/2.5*sin(millis()/500.0)*sin(millis()/1027.0)+50*sin(millis()/220.9)
        //patternX=wave(width/4,.3,0,0)
       patternY=height/2.5*sin(millis()/323.0)*sin(millis()/533.0)+50*sin(millis()/277.0);
...
function drawLines(){
  if (reflection){
    if (!machine){
        ...
        if (!machine){
         if (i%(linVariant*smoothDelay)==0) {
           for (var k=0;k<numRotate;k++){
             rotate(TWO_PI/numRotate);
             bezier(pointOneX, pointOneY, interpSecondX, interpSecondY,  pointThreeX, pointThreeY,  patternX, patternY);
           }
         }
       }  
...
function drawNucleus(nucleusColor){
    var res=10;
    //strokeWeight(2);
    //fill(0);
    noStroke();
