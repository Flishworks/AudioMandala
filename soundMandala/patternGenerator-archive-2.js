...
function genPattern(){
  ...
  if((drawMode&&mouseIsPressed)||!drawMode){ //dont continuousely draw non-moving dots
    for (var n=0;n<10;n++){ //draw line multiple times for various line effects
      if (n==0&&glow){
        //for glowing lines
      ...
      if (i%(linVariant*smoothDelay)==0&&!machine) { //for line effect.
          drawBezier();
      }
      else if(machine){
        drawStraight();
...
function drawBezier(){//for bezier
            switch (numLines){
              case 4:
                //8 lines
